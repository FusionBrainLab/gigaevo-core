...
- **`write/`** — `MemoryWriter` facade: extract valid parent→child records → librarian LLM reconciles each diff into NEW/DUPLICATE/MERGE cards → author program exemplars → restamp gain events → configured eviction sweep (`NullEvictor` by default) → throttled background near-duplicate consolidation. `write/` never imports `read/`; the write-side eviction surface (the `Evictor` Protocol + `NullEvictor`) is self-contained in `write/`.
