...
# ---------------------------------------------------------------------------
# StageError surrogate-scrubbing regression guards
#
# When a bandit-wrapped LLM call fails inside an agent, the mutation operator
# catches the exception and constructs a ``StageError`` from it. Provider
# exception text occasionally contains lone UTF-16 surrogate code points
# (tokenizer artefacts) — legal in Python ``str`` but rejected by
# ``orjson.dumps`` (TypeError: str is not valid UTF-8) and by PostgreSQL TEXT
# columns. The downstream Redis-write path (``gigaevo/database/redis_program_storage.py``
# → ``gigaevo/utils/json.py.dumps`` → ``orjson``) crashes the engine in that
# case. ``StageError``'s field validators must scrub surrogates to U+FFFD at
# the boundary so every downstream consumer is safe.
# ---------------------------------------------------------------------------


class TestStageErrorSurrogateScrubbing:
    """Positive + negative coverage for the surrogate-scrub validators."""

    def test_lone_high_surrogate_in_message_replaced(self) -> None:
        # Positive: lone high surrogate U+D800 is scrubbed to U+FFFD so
        # downstream orjson.dumps does not crash.
        from gigaevo.utils.json import dumps as ujson_dumps

        se = StageError(type="RuntimeError", message="hostile\ud800msg")
        assert "\ud800" not in se.message
        ujson_dumps(se.model_dump())  # must not raise TypeError

    def test_surrogate_pair_in_traceback_replaced(self) -> None:
        # Negative-direction: a lone low surrogate AND a stray high surrogate
        # at non-adjacent positions both get scrubbed, and the full
        # ``StageError`` round-trips through ``model_dump_json`` afterward.
        tb = "Traceback (most recent call last):\n  \udc00 here\nRuntimeError: \ud83d"
        se = StageError(type="RuntimeError", message="ok", traceback=tb)
        assert se.traceback is not None
        for ch in se.traceback:
            cp = ord(ch)
            assert not (0xD800 <= cp <= 0xDFFF), f"surrogate U+{cp:04X} survived"
        se.model_dump_json()  # must not raise

    def test_real_astral_emoji_preserved(self) -> None:
        # Negative: a real BMP-paired astral emoji is NOT a lone surrogate
        # and must pass through untouched (Python stores 😀 as a single code
        # point U+1F600, no surrogates involved).
        se = StageError(type="RuntimeError", message="error 😀 happened")
        assert se.message == "error 😀 happened"

    def test_from_exception_with_surrogate_str_safe(self) -> None:
        # End-to-end: bandit-wrapped LLM exception → StageError.from_exception
        # → model_dump → utils.json.dumps. Pre-fix this raised TypeError
        # from orjson.
        from gigaevo.utils.json import dumps as ujson_dumps

        exc = RuntimeError("provider says \ud800 oops")
        se = StageError.from_exception(exc, stage="LLMCall")
        ujson_dumps(se.model_dump())  # must not raise

    def test_program_with_surrogate_stage_error_serializes(self) -> None:
        # End-to-end through Program: the typical Redis-write path is
        # ``RedisProgramStorage.add`` → ``program.to_dict()`` →
        # ``gigaevo.utils.json.dumps``. Confirms the validator runs early
        # enough that the stored Program is orjson-safe.
        from gigaevo.utils.json import dumps as ujson_dumps

        prog = _make_program()
        prog.stage_results["LLMCall"] = ProgramStageResult.failure(
            error=StageError.from_exception(
                RuntimeError("a\ud800b"), stage="LLMCall"
            )
        )
        ujson_dumps(prog.to_dict())  # must not raise


class TestStageErrorScrubIdempotent:
    """``_scrub_surrogates`` must be idempotent: applying twice yields the
    same string. Validators run on every model construction (including
    ``model_validate`` after a JSON round-trip), so non-idempotent scrubbing
    would corrupt restored records."""

    def test_scrubbed_value_passes_through_unchanged(self) -> None:
        from gigaevo.programs.core_types import _scrub_surrogates

        once = _scrub_surrogates("a\ud800b")
        twice = _scrub_surrogates(once)
        assert once == twice
        assert "\ud800" not in once

    def test_validator_idempotent_on_revalidation(self) -> None:
        # Build, dump, re-validate: must not double-scrub.
        se = StageError(type="RuntimeError", message="a\ud800b")
        first = se.message
        revalidated = StageError.model_validate(se.model_dump())
        assert revalidated.message == first
        assert "�" in revalidated.message
