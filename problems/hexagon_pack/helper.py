...
def get_hexagon_vertices(center: np.ndarray, angle: float, side: float = 1.0) -> np.ndarray:
...
def compute_outer_hex_side_length(centers: np.ndarray, angles: np.ndarray, unit_side: float = 1.0) -> float:
    ...
    directions = np.stack([np.cos(projection_angles), np.sin(projection_angles)], axis=-1)
...
def _segments_intersect(p1, p2, q1, q2) -> bool:
    ...
    def _on_segment(a, b, c):
        return (
            min(a[0], b[0]) - 1e-8 <= c[0] <= max(a[0], b[0]) + 1e-8 and
            min(a[1], b[1]) - 1e-8 <= c[1] <= max(a[1], b[1]) + 1e-8
        )
    ...
    if o1 == 0 and _on_segment(p1, p2, q1) and not is_touching_endpoint(q1, p1) and not is_touching_endpoint(q1, p2):
        return True
    if o2 == 0 and _on_segment(p1, p2, q2) and not is_touching_endpoint(q2, p1) and not is_touching_endpoint(q2, p2):
        return True
    if o3 == 0 and _on_segment(q1, q2, p1) and not is_touching_endpoint(p1, q1) and not is_touching_endpoint(p1, q2):
        return True
    if o4 == 0 and _on_segment(q1, q2, p2) and not is_touching_endpoint(p2, q1) and not is_touching_endpoint(p2, q2):
        return True
...
def check_hexagon_overlap_two(center: np.ndarray, angle: float, other_center: np.ndarray, other_angle: float) -> bool:
