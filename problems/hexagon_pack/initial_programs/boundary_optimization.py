    ...
    for i in range(len(centers)):
        for j in range(i+1, len(centers)):
            ...
            if dist < min_distance:
                ...
                centers[j] = centers[i] + direction / np.linalg.norm(direction) * min_distance * 1.1
    
    # Rotation strategy: optimize for boundary contact
    angles = np.array([0.0, np.pi/3, 2*np.pi/3,  # Core rotations
                      np.pi/8, -np.pi/8, np.pi/4, -np.pi/4, 3*np.pi/8, -3*np.pi/8, np.pi/2, -np.pi/2])  # Boundary
