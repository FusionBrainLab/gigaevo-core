...
# The Improver machinery above is unchanged. _direct_solve() below replaces the
...
_DIRECT_IMPROVER_CLS = Improver
_DIRECT_TIME_BUDGET_S = 20.0  # stay under the ~30 s per-size slice
...
def _direct_solve(hex_num: int = 11, seed: int = 0):
    import time
...
class Solver:
    def __init__(self, hex_num: int = 11, seed: int = 0):
        self.hex_num = int(hex_num)
        self.seed = int(seed)

    def solve(self, hex_num: int, seed=None):
        return _direct_solve(
            hex_num=int(hex_num), seed=self.seed if seed is None else int(seed)
        )
