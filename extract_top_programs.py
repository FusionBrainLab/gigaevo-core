...
class TopProgramExtractor:
    ...
    def filter_programs_by_fitness(self, programs: List[Program], fitness_metric: str = "fitness") -> List[Program]:
        """Filter programs that have valid fitness values."""
        fitness_col = f"metric_{fitness_metric}" if not fitness_metric.startswith("metric_") else fitness_metric
        ...
        logger.info(f"📈 Found {len(valid_programs)} programs with valid '{fitness_metric}' metric out of {len(programs)} total")
        return valid_programs
    
    def get_top_programs(self, programs: List[Program], top_n: int, fitness_metric: str = "fitness") -> List[Program]:
        """Get top N programs sorted by fitness."""
        if not programs:
            return []
        
        # Sort programs by fitness (descending - higher fitness is better)
        sorted_programs = sorted(programs, key=lambda p: p.metrics.get(fitness_metric, -float('inf')), reverse=True)
        
        # Get top N
        top_programs = sorted_programs[:top_n]
        
        logger.info(f"🏆 Selected top {len(top_programs)} programs by '{fitness_metric}' metric")
        ...
        if top_programs:
            ...
            logger.info(f"Fitness range: {worst_fitness:.4f} to {best_fitness:.4f}")
        
        return top_programs
    
    def save_programs_to_folder(self, programs: List[Program], output_folder: Path, fitness_metric: str = "fitness") -> None:
...
'''
        ...
        with open(summary_file, 'w', encoding='utf-8') as f:
            ...
            f.write(f"Redis: {self.redis_host}:{self.redis_port}/{self.redis_db}\n\n")
            
            for i, program in enumerate(programs, 1):
                fitness_value = program.metrics.get(fitness_metric, 0.0)
                f.write(f"{i:2d}. Program {program.id[:12]}... | Fitness: {fitness_value:.4f} | Gen: {program.generation or 'N/A'} | State: {program.state}\n")
                
                # Add lineage info if available
                if program.lineage and program.lineage.parents:
                    f.write(f"    Parents: {len(program.lineage.parents)} | Mutation: {program.lineage.mutation or 'N/A'}\n")
                
                # Add other metrics if available
                other_metrics = {k: v for k, v in program.metrics.items() if k != fitness_metric}
                if other_metrics:
                    metrics_str = ', '.join([f'{k}: {v:.3f}' for k, v in list(other_metrics.items())[:3]])
    ...
    async def extract_and_save_top_programs(self, output_folder: Path, top_n: int = 10, fitness_metric: str = "fitness") -> None:
        ...
        valid_programs = self.filter_programs_by_fitness(all_programs, fitness_metric)
        if not valid_programs:
            logger.error(f"❌ No programs found with valid '{fitness_metric}' metric")
            return
        
        # Get top N programs
        top_programs = self.get_top_programs(valid_programs, top_n, fitness_metric)
        if not top_programs:
            logger.error("❌ No top programs selected")
            return
        
        # Save to folder
        self.save_programs_to_folder(top_programs, output_folder, fitness_metric)
        
        logger.info(f"🎉 Successfully extracted and saved {len(top_programs)} top programs!")
...
async def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="Extract top programs by fitness from Redis")
    parser.add_argument("--output-folder", type=str, required=True, help="Directory to save the program files")
    parser.add_argument("--top-n", type=int, default=10, help="Number of top programs to extract (default: 10)")
