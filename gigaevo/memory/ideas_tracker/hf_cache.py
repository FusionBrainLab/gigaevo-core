...
The write path's neighbor source loads a sentence-transformers model, which
follows ``HF_HOME`` and related env vars. Shared clusters often point these at
NFS roots that this user cannot write, breaking the embedding download. This
module clears unwritable entries and falls back to ``~/.cache/huggingface``. It
is the one place in the write path that touches ``os.environ`` (see
``tests/memory/test_no_env_in_memory.py``); it runs lazily when the stack is
built, not at import.
"""
