...
| `read/projection.py` | `AuctionCandidateProjector`: attaches context key, EB/static cold prior, no-card evidence, and per-event credit×staleness EV weights to each auction candidate |
| `read/prior.py` | Cold priors; context cohorts are native-task-only and global cohorts pool hard signs |
...
| `read/staleness.py` | `bank_cycle_event_weights`: the shared staleness mechanism, `w_i = 2^(-s_i/H)` (`s_i` = native bank stamps newer than event `i`'s own stamp, `H` = task population × `half_life_cycles`) |
...
| `write/eviction.py` | Evictor Protocols/policies and the shared hard-sign foreign-retention veto |
...
the bank up from disk (this is how a `memory=reader` run consumes a bank a prior
