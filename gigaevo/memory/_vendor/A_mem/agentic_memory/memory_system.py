...
class AgenticMemorySystem:
    ...
    def __init__(
        ...
        Args:
            ...
            llm_backend: LLM backend to use (openai/ollama/custom)
            llm_model: Name of the LLM model
            evo_threshold: Number of memories before triggering evolution
            enable_evolution: Whether to run LLM-based process_memory on add_note
            api_key: API key for the LLM service
            llm_service: Pre-configured LLMService instance to use instead of built-in controllers
        """
        self.memories = {}
        ...
        self.llm_controller = LLMController(
            llm_backend, llm_model, api_key, llm_service=llm_service
        )
    ...
    def _parse_llm_json(self, response: Any) -> Any:
        ...
        try:
            ...
            try:
                ...
                raise ValueError(
                    f"Could not parse LLM response as JSON. Raw head={text[:200]!r}"
                ) from e
    ...
    def analyze_content(self, content: str) -> dict:
        ...
        try:
            # print("\n[LLM DEBUG][analyze_content] Prompt sent to LLM:\n" + prompt)
    ...
    def process_memory(self, note: MemoryNote) -> tuple[bool, MemoryNote]:
        ...
        try:
            ...
            try:
                # print(f"\n[LLM DEBUG][process_memory] Prompt for note {note.id}:\n{prompt}")
                response = self.llm_controller.llm.get_completion(
                    prompt,
                    response_format={
                        "type": "json_schema",
