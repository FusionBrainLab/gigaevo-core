...
class DAGSpec(BaseModel):
    ...
    nodes: Dict[str, Callable[[], Stage]] = Field(..., description="Stage factories by name")
    edges: Dict[str, List[str]] = Field(..., description="DAG edges mapping source to destinations")
    entry_points: Optional[List[str]] = Field(None, description="Entry point stage names")
    exec_order_deps: Optional[Dict[str, List[ExecutionOrderDependency]]] = Field(
    ...
    def validate_edges(cls, v: Dict[str, List[str]], info) -> Dict[str, List[str]]:
        ...
        unknown = {dst for dsts in v.values() for dst in dsts if dst not in nodes}
        if unknown:
    ...
    def visualize(
        ...
        if self.exec_order_deps:
            for stage_name, deps in self.exec_order_deps.items():
                for dep in deps:
                    G.add_edge(dep.stage_name, stage_name, edge_type='exec_order', condition=dep.condition)
        ...
        try:
            ...
            print("Warning: Graph contains cycles, using spring layout instead of layered layout")
        ...
        regular_edges = [(u, v) for (u, v, d) in G.edges(data=True) if d.get('edge_type') == 'regular']
        exec_order_edges = [(u, v) for (u, v, d) in G.edges(data=True) if d.get('edge_type') == 'exec_order']
        ...
        if show_legend:
            legend_elements = [
                ...
                plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='#2E8B57', 
                          markersize=10, label='Entry Points'),
                plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='#FF8C00', 
                          markersize=10, label='Execution Order Dependencies'),
                plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='#87CEEB', 
                          markersize=10, label='Regular Nodes'),
                # Edge types
                plt.Line2D([0], [0], color='gray', linewidth=2, label='Regular Dependencies'),
                plt.Line2D([0], [0], color='#228B22', linewidth=3, linestyle='--', 
                          label='On Success Dependencies'),
                plt.Line2D([0], [0], color='#DC143C', linewidth=3, linestyle=':', 
        ...
        plt.title('DAG Specification Visualization (Topologically Sorted)', fontsize=16, fontweight='bold', pad=20)
        plt.tight_layout()
    ...
    def _create_layered_layout(self, G: nx.DiGraph, topo_order: List[str]) -> List[List[str]]:
        """
    ...
    def _position_nodes_in_layers(self, layers: List[List[str]], figsize: tuple[int, int]) -> Dict[str, tuple[float, float]]:
        """
        ...
        for layer_idx, layer in enumerate(layers):
            ...
            if len(layer) == 1:
                ...
                spacing = usable_width / (len(layer) - 1) if len(layer) > 1 else 0
