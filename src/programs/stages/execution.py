...
class RunPythonCode(Stage):
    ...
    async def _execute_stage(
        ...
        logger.debug(f"[{self.stage_name}] Program {program.id}: Executing Python code")
        ...
        try:
            ...
            if self.enable_sandboxing:
                result = await self._run_sandboxed(exec_code, started_at, program.id)
            else:
                result = await run_python_snippet(
                    exec_code, started_at, timeout=self.timeout, stage_name=self.stage_name
                )
    ...
    async def _set_resource_limits(self) -> None:
        try:
            ...
            logger.warning(f"[{self.stage_name}] Failed to set resource limits: {e}")
...
class RunValidationStage(RunPythonCode):
    def __init__(
        ...
        **kwargs,
        ...
        if not self.validator_path.exists():
            raise ValidationError(f"Validator file not found: {self.validator_path}")
    ...
    async def _execute_stage(
        ...
        logger.debug(f"[{self.stage_name}] Program {program.id}: Running validation against {self.data_to_validate_stage}")
