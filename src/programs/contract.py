...
class ProgramContractChecker:
    ...
    def check_program(self, program: Program) -> Tuple[bool, List[ContractViolation]]:
        """
        ...
        if not program.metrics:
            ...
            # Check required keys
            missing_keys = self.required_behavior_keys - set(program.metrics.keys())
            if missing_keys:
                violations.append(ContractViolation(
    ...
    def check_programs_batch(self, programs: List[Program]) -> Dict[str, Tuple[bool, List[ContractViolation]]]:
    ...
    def generate_contract_report(self, programs: List[Program]) -> str:
        ...
        if self.required_behavior_keys:
            report.append(f"Required Behavior Keys: {sorted(self.required_behavior_keys)}")
        else:
            report.append("Required Behavior Keys: None (any non-empty metrics accepted)")
        ...
        if violation_summary:
            ...
            for violation_type, violations in violation_summary.items():
                ...
                for violation in violations[:3]:  # Show first 3 examples
                    report.append(f"  • {violation.program_id}: {violation.description}")
                    report.append(f"    💡 {violation.suggestion}")
                if len(violations) > 3:
                    report.append(f"  ... and {len(violations) - 3} more programs")
        ...
        if valid_count > 0:
            ...
            if len(valid_programs) > 5:
                report.append(f"  ... and {len(valid_programs) - 5} more valid programs")
...
def diagnose_empty_metrics(program: Program) -> str:
    ...
    diagnosis.append("-" * 50)
    diagnosis.append(f"Program Status: is_complete={program.is_complete}, is_discarded={program.is_discarded}")
    ...
    if failed_stages:
        ...
        diagnosis.append("   💡 Check stage error messages and fix validation logic")
    
    if skipped_stages:
        diagnosis.append("2. ⏭️  Critical stages were skipped due to dependencies")
        diagnosis.append("   💡 Check DAG dependencies and ensure prerequisite stages succeed")
    
    if not any('metric' in stage.lower() or 'validat' in stage.lower() for stage in completed_stages):
        diagnosis.append("3. 🔍 No metrics or validation stages found")
        diagnosis.append("   💡 Ensure your DAG includes UpdateMetricsStage or FactoryMetricsStage")
    
    diagnosis.append("\nRECOMMENDED ACTIONS:")
    diagnosis.append("• Use FactoryMetricsStage instead of UpdateMetricsStage for fallback metrics")
...
def create_contract_summary(required_behavior_keys: Optional[Set[str]] = None) -> str:
    ...
    if required_behavior_keys:
        ...
        summary.append(f"   • These keys must exist in metrics: {sorted(required_behavior_keys)}")
        summary.append("   • Missing any required key = automatic rejection")
        summary.append("   • Configure your DAG to compute these metrics")
    else:
        summary.append("3. ✅ Required Behavior Keys:")
        summary.append("   • No specific keys required (any non-empty metrics accepted)")
        summary.append("   • Consider setting required_behavior_keys in EngineConfig")
