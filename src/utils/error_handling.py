    ...
    for attempt in range(config.max_attempts):
        try:
            ...
            if attempt == config.max_attempts - 1:
                logger.error(f"Failed {operation_name} after {config.max_attempts} attempts: {e}")
                raise
            ...
            logger.warning(f"Attempt {attempt + 1} failed for {operation_name}, retrying in {delay:.2f}s: {e}")
...
class HealthChecker:
    ...
    async def check_health(self, component: str, health_check: Callable[[], Awaitable[bool]]) -> bool:
    ...
    async def _update_health_status(self, component: str, is_healthy: bool, error: Optional[str]):
            ...
            async def protected_call():
                if circuit_breaker_name:
                    if circuit_breaker_name not in _circuit_breakers:
                        _circuit_breakers[circuit_breaker_name] = CircuitBreaker(
