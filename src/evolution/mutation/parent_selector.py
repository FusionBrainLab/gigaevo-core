...
class ParentSelector(ABC):
    ...
    @abstractmethod
    def select_parents(self, available_parents: List[Program]) -> Optional[List[Program]]:
...
class AllCombinationsParentSelector(ParentSelector):
    ...
    def select_parents(self, available_parents: List[Program]) -> Optional[List[Program]]:
        ...
        expected_combinations = list(combinations(available_parents, self.num_parents))
...
class WeightedRandomParentSelector(ParentSelector):
    ...
    def __init__(self, num_parents: int = 1, fitness_key: str = "fitness", higher_is_better: bool = True):
        self.num_parents = num_parents
    ...
    def select_parents(self, available_parents: List[Program]) -> Optional[List[Program]]:
        ...
        for parent in available_parents:
            if self.fitness_key in parent.metrics:
                ...
                if self.higher_is_better:
                    ...
                    fitnesses.append(max(0, -fitness))  # Invert for minimization
        ...
        for _ in range(self.num_parents):
            ...
            chosen = random.choices(remaining_parents, weights=remaining_fitnesses, k=1)[0]
