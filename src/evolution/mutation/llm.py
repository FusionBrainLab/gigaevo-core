...
class LLMMutationOperator(MutationOperator):
    ...
    async def mutate_single(self, available_parents: List[Program], parent_selector: ParentSelector) -> Optional[MutationSpec]:
        ...
        if not selected_parents:
            logger.warning(f"[LLMMutationOperator] Parent selector returned no parents")
            return None
        ...
        try:
            ...
            if self.mutation_mode == "diff":
                ...
                try:
                    ...
                    raise MutationError(f"Diff application failed: {diff_error}") from diff_error
            else:
            ...
            mutation_spec = MutationSpec(code=final_code.strip(), parents=selected_parents, name=label)
            logger.debug(f"[LLMMutationOperator] Generated mutation from {len(selected_parents)} parents")
...
"""
        ...
        return self.user_prompt_template.format(count=len(parents), parent_blocks="\n\n".join(parent_blocks))
