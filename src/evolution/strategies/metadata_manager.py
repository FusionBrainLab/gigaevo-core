...
class MetadataManager:
    ...
    async def set_current_island(self, program: Program, island_id: str) -> None:
        try:
            ...
            logger.debug(f"MetadataManager: Set current_island={island_id} for program {program.id}")
        except Exception as e:
            logger.warning(f"MetadataManager: Failed to set current_island for program {program.id}: {e}")
    ...
    async def clear_current_island(self, program: Program) -> None:
        try:
            if program.metadata.get("current_island"):
                ...
                await self.program_storage.update(program)
                logger.debug(f"MetadataManager: Cleared current_island for program {program.id}")
        except Exception as e:
            logger.warning(f"MetadataManager: Failed to clear current_island for program {program.id}: {e}") 