...
class ScoreArchiveRemover(ArchiveRemover):
    ...
    def __call__(self, programs: List[Program], max_size_to_keep: int) -> List[Program]:
        ...
        try:
            ...
            logger.warning(f"Score-based removal failed: {e}. Falling back to random removal.")
...
class FitnessArchiveRemover(ScoreArchiveRemover):
    ...
    def score(self, program: Program) -> float:
        ...
        if not hasattr(program, 'metrics') or program.metrics is None:
            return float('-inf') if self.fitness_key_higher_is_better else float('inf')
        
        if self.fitness_key not in program.metrics:
            logger.warning(f"Fitness key {self.fitness_key} not found in program {program.id} metrics. Available keys: {list(program.metrics.keys())}")
            return float('-inf') if self.fitness_key_higher_is_better else float('inf')
        
        fitness_value = program.metrics.get(self.fitness_key, 0.0)
        try:
            return float(fitness_value) if self.fitness_key_higher_is_better else -float(fitness_value)
        except (ValueError, TypeError):
            logger.warning(f"Invalid fitness value for program {program.id}: {fitness_value}")
            return float('-inf') if self.fitness_key_higher_is_better else float('inf')
...
class ParetoFrontArchiveRemover(ArchiveRemover):
    ...
    def __init__(self, fitness_keys: list[str], tie_breaker: Callable[[Program], float], fitness_key_higher_is_better: dict[str, bool] | None = None):
    ...
    def __call__(self, programs: List[Program], max_size_to_keep: int) -> List[Program]:
        ...
        try:
            ...
            logger.warning(f"Pareto front removal failed: {e}. Falling back to random removal.")
            num_to_remove = len(programs) - max_size_to_keep
            return random.sample(programs, num_to_remove)
    ...
    def order_candidates(self, programs: List[Program]) -> List[Program]:
        ...
        try:
            fitness_vectors = [extract_fitness_values(p, self.fitness_keys, self.fitness_key_higher_is_better) for p in programs]
        except Exception as e:
            logger.warning(f"Failed to extract fitness values for Pareto sorting: {e}")
...
class ParetoFrontArchiveRemoverDropOldest(ParetoFrontArchiveRemover):
    ...
    def __init__(self, fitness_keys: list[str], fitness_key_higher_is_better: dict[str, bool] | None = None):
        super().__init__(fitness_keys, lambda x: x.created_at.timestamp(), fitness_key_higher_is_better)