...
class QualityDiversityMetrics(BaseModel):
    ...
    coverage: float = Field(ge=0.0, le=1.0, description="Fraction of cells filled (0-1)")
...
class BehaviorSpace(BaseModel):
    ...
    def validate_feature_bounds(cls, v):
        for key, (min_val, max_val) in v.items():
            if min_val > max_val:
                raise ValueError(f"Invalid bounds for {key}: min ({min_val}) must be <= max ({max_val})")
        return v
    ...
    def validate_consistency(self):
        ...
        for key in self.behavior_keys:
            if self.get_binning_type(key) == BinningType.LOGARITHMIC:
                ...
                if min_val <= 0:
                    raise ValueError(f"Logarithmic binning requires positive bounds for {key}, got min={min_val}")
        
        if self.total_cells > 1_000_000:
            logger.warning(f"Large behavior space with {self.total_cells:,} cells may impact performance")
    ...
    def get_cell(self, metrics: Dict[str, float]) -> Tuple[int, ...]:
        ...
        for behavior_key in self.behavior_keys:
            if behavior_key not in metrics:
                raise KeyError(f"Missing required behavior key '{behavior_key}' in program metrics")
    ...
    def _map_value_to_coordinate(self, behavior_key: str, value: float) -> int:
        ...
        if binning_type == BinningType.LINEAR:
            ...
            # Fallback to linear
            logger.warning(f"Unknown binning type {binning_type} for {behavior_key}, using linear")
            coordinate = self._linear_binning(value, min_val, max_val, num_bins)
        
        # Clamp to valid range
        return max(MIN_COORDINATE, min(coordinate, num_bins - 1))

    def _linear_binning(self, value: float, min_val: float, max_val: float, num_bins: int) -> int:
    ...
    def _logarithmic_binning(self, value: float, min_val: float, max_val: float, num_bins: int) -> int:
    ...
    def _square_root_binning(self, value: float, min_val: float, max_val: float, num_bins: int) -> int:
