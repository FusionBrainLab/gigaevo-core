    ...
    try:
        ...
        while persisted < limit:
            try:
                ...
                mutation_spec = await mutator.mutate_single(elites, parent_selector)
                
                if mutation_spec is None:
                    # No mutation could be generated
                    if not parent_selector.has_more_selections():
                        logger.info(f"[mutation] Parent selector exhausted after {persisted} mutations")
                        break
                    else:
                        logger.debug(f"[mutation] Failed to generate mutation, but parent selector has more options")
                        continue
                ...
                logger.debug(f"[mutation] Persisted mutation {persisted}/{limit}: {mutation_spec.name}")
                
                # Check if we should continue (for exhaustive selectors)
                if not parent_selector.has_more_selections():
                    logger.info(f"[mutation] No more parent selections available after {persisted} mutations")
                    break
                    
            except Exception as exc:
                logger.error(f"[mutation] Failed to generate/persist mutation {persisted + 1}: {exc}")
                
                # For exhaustive selectors, we might want to continue with remaining combinations
                if parent_selector.has_more_selections():
                    logger.debug("[mutation] Continuing with next parent selection")
                    continue
                else:
                    logger.info("[mutation] No more parent selections available after error")
                    break

    except Exception as exc:  # pragma: no cover
        logger.error(f"[mutation] Mutation generation failed: {exc}.")

    logger.info(f"[mutation] Created {persisted} mutations (immediately persisted)")
    return persisted 